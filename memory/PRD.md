...
- REFINE (2026-07-09i) Alte BTTS-Bet-Builder zeigten weiter zwei "{Team} Über 0.5 Tore"-Chips (vor dem Generierungs-Fix gepostet). Migration `_migrate_stars_and_categories()` erweitert: bei is_parlay-Picks mit ≥2 team_o05-Legs werden die Display-Chips zu EINEM "Beide Teams treffen @<Produkt>"-Chip zusammengefasst (+ verbleibende Legs), rein visuell — combo_legs & Gesamtquote unverändert (Abrechnung bleibt korrekt via team_o05-Grading). VERIFIZIERT: Test-Pick Dila Gori/SS Virtus → Chips ['Beide Teams treffen'@1.69, 'Über 1.5 Tore'@1.40], Quote 2.37 unverändert. Läuft beim Start (auch Produktion).
