...
async def get_opencase(user: dict = Depends(get_current_user)):
    fresh = await db.users.find_one({"id": user["id"]})
    return {"jar_ids": (fresh.get("open_case") or [])[:3]}


@api_router.put("/jars/opencase")
async def set_opencase(inp: OpenCaseInput, user: dict = Depends(get_current_user)):
