# TipJar — MEMORY.md
_Konsolidiert 2026-08-14. Projektstand, Verlauf, Test-Zugang. Root-only (kein memory/ Ordner)._
...
## 2026-08-09 (Fork) — Sprechblase "unfickbar" + neuer Hard-2:2-Bereich
- Master-Sprechblase (master_avatar_calls) neu: KEINE riskanten "Über 0.5 1.HZ"/Torschützen-Calls mehr (fallen bei 0-0/2-2 durch). Jetzt Rotation aus 3 bombensicheren Aussagen: "{Favorit} gewinnt mindestens eine Halbzeit" (settlement half_any, deterministisch), "Doppelte Chance {Favorit}", "Über 1,5 Tore" (nur goal-friendly). Alle settlebar. Getestet: postet Porto/Benfica/Sheriff mit den neuen Märkten (Screenshot Blase aktiv & swipebar).
- NEU: Master-Bereich "🎯 Hard 2:2" (master_category=hard, master_hard_2_2). EINE tägliche Kombi aus exakten 2:2 für Fallen-Spiele (Favorit den die Masse auf Sieg tippt, endet aber remis mit Toren wie PSV/Sporting/Darmstadt). Filter: fav_prob 50-72, ph>=2 & pa>=2, tot<=5, |ph-pa|<=1, keine Friendlies, Cap 6 Beine. Getestet: 3er-Kombi @2197, API mcat=hard liefert Combo, Tab rendert.
...
# Test Credentials — TipJar

## Admin account
- Email: `admin@tipjar.com`
- Password: `TipJarAdmin2026!`

Use this account for any auth/admin testing (settle, status changes, admin endpoints).

## Expert / Mailbox notes
