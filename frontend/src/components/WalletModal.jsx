...
const TEXTS = {
  en: { title: "Your Coins", available: "AVAILABLE", balance: "BALANCE", buy: "Top Up", gift: "Gift", redeem: "Cash Out", credits: "Coins", buying: "Redirecting to payment...", gifted: "Gifted", redeemOk: "Payout requested", redeemBtn: "Withdraw €50", notenough: "Not enough for payout", redeemInfo: "10,000 Coins = 1 packet = €50 base. +2.5% bonus per packet, max €62.50 after 10 packets. 1 Coin = 0.5 cent. Sale price always higher than payout — no loss possible. Battery >2000 + 1000+ members for Live.", lowBattery: "Battery must be over 2000 to cash out.", buyDisabled: "", creditsPurpose: "Coins are for gifting other members. Posting and rating is free. Battery max 2500.", giftTo: "Username", amount: "Amount", fee: "Gift fee: 0 coins. 100% arrives.", send: "Send", popular: "POPULAR", bestValue: "BEST VALUE", bestDeal: "BEST DEAL -61%", bonus: "+{n} Bonus" },
  de: { title: "Deine Münzen", available: "VERFÜGBAR", balance: "GUTHABEN", buy: "Aufladen", gift: "Verschenken", redeem: "Auszahlen", credits: "Münzen", buying: "Weiter zur Zahlung...", gifted: "Verschenkt", redeemOk: "Auszahlung angefordert", redeemBtn: "50€ auszahlen", notenough: "Noch nicht genug für Auszahlung", redeemInfo: "10.000 Coins = 1 Packet = 50€ Base. +2,5% Bonus pro Packet, max 62,50€ nach 10 Packets. 1 Coin = 0,5 Cent. Verkauf immer teurer als Auszahlung — kein Verlust möglich. Battery >2000 + 1000+ Members für Live.", lowBattery: "Batterie muss über 2000 sein, um auszuzahlen.", buyDisabled: "", creditsPurpose: "Münzen sind zum Verschenken. Posten & Bewerten ist kostenlos. Batterie max 2500.", giftTo: "Benutzername", amount: "Anzahl", fee: "Gebühr: 0 Münzen. 100% kommt an.", send: "Senden", popular: "BELIEBT", bestValue: "BESTER WERT", bestDeal: "BEST DEAL -61%", bonus: "+{n} Bonus" },
    ...
    <Modal open={open} onClose={onClose} title={tx("title")} testId="wallet-modal">
      <div className="grid grid-cols-2 gap-3 mb-5">
        ...
        <div className="rounded-xl bg-void border border-elevated p-4">
          ...
          <p className="text-[10px] text-zinc-500 mt-1">{pct.toFixed(2)}% • {rc} / {REDEEM_THRESHOLD}</p>
        </div>
      </div>
      ...
      {tab === "redeem" && (
        <div data-testid="redeem-panel">
          ...
          <p className="font-mono text-sm text-white mb-4">{rc} / {REDEEM_THRESHOLD} ({pct.toFixed(2)}%)</p>
          <button data-testid="redeem-btn" onClick={redeem} disabled={busy || rc < REDEEM_THRESHOLD || available < 2000} className={btnPrimary + " flex items-center justify-center gap-2"}>
            <Check size={18} /> {rc >= REDEEM_THRESHOLD ? tx("redeemBtn") : tx("notenough")}
          </button>
          {available < 2000 && <p data-testid="redeem-low-battery" className="text-[11px] text-amber-400 mt-2 text-center">{tx("lowBattery")}</p>}
        </div>
      )}
    </Modal>
  );
