...
// Owner 2026-08: community "battery". Shows real user credits out of the full charge (2500 CR).
// Colour by charge level: <25% red, <50% amber, <75% lime, else green.
export default function CreditBattery({ current = 0, max = 2500, label = "TipJar Charge" }) {
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold uppercase tracking-[0.2em] text-zinc-300">{label}</span>
        </div>
        <span data-testid="credit-battery-value" className="font-mono text-sm font-bold" style={{ color }}>
          {cur.toLocaleString()}/{cap.toLocaleString()} CR
        </span>
      </div>
      {/* battery body */}
      <div className="flex items-center gap-1.5">
        <div className="relative flex-1 h-7 rounded-md border-2 border-white/25 bg-void/70 overflow-hidden">
          <div
            ...
            data-testid="credit-battery-fill"
          />
      ...
      <p className="mt-2 text-[11px] text-zinc-500 leading-snug">
        Die App läuft auf Credits. Jeder lädt mit — bei voller Ladung (2500 CR) schalten wir neue Features frei.
      </p>
    </div>
  );
}
